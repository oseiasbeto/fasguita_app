  ...
  <div class="min-h-screen bg-white flex flex-col">
    ...
    <div class="flex-1 flex flex-col justify-center px-6 pb-12">
      <div class="w-full">
        ...
        <form v-if="!showOtpInput" @submit.prevent="handleRegister" class="space-y-8" autocomplete="off">
          <!-- Nome Completo -->
          <div>
            ...
            <div class="relative">
              <input v-model="fullName" type="text" placeholder="João Silva" required minlength="3" autocomplete="name"
                class="w-full px-6 py-5 text-xl border-2 border-gray-300 rounded-2xl focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-100 transition-all"
                :class="{
                  'border-red-500 focus:border-red-500 focus:ring-red-100': fullNameTouched && !fullNameValid,
                  'border-green-500 focus:border-green-500 focus:ring-green-100': fullNameValid && fullNameTouched
                }" @blur="fullNameTouched = true; validateFullName()" />

              <!-- Ícone de feedback -->
              <div class="absolute inset-y-0 right-5 flex items-center">
                <svg v-if="fullNameValid && fullNameTouched" class="w-7 h-7 text-green-500" fill="none"
                  viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                </svg>
                <svg v-if="fullNameTouched && !fullNameValid" class="w-7 h-7 text-red-500" fill="none"
                  viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
            ...
            <p v-if="fullNameError && fullNameTouched" class="mt-2 text-sm text-red-600 font-medium">
              {{ fullNameError }}
            </p>
          </div>
          ...
          <div>
            ...
            <div class="relative">
              <input v-model="confirmPassword" :type="showConfirmPassword ? 'text' : 'password'" required
                class="w-full px-6 py-5 text-xl border-2 border-gray-300 rounded-2xl focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-100 transition-all pr-16"
                :class="{
                  'border-red-500 focus:border-red-500 focus:ring-red-100': confirmPassword.length > 0 && confirmPassword !== password,
                  'border-green-500 focus:border-green-500 focus:ring-green-100': confirmPassword.length > 0 && confirmPassword === password
                }" />

              <button type="button" @click="showConfirmPassword = !showConfirmPassword"
                class="absolute inset-y-0 right-5 flex items-center text-gray-500 hover:text-gray-700 focus:outline-none">
                <svg v-if="showConfirmPassword" class="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                  stroke-width="2">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                  <path stroke-linecap="round" stroke-linejoin="round"
                    d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
            ...
            <p v-if="confirmPassword.length > 0 && confirmPassword !== password"
        ...
        <div v-else class="space-y-8">
          <div>
            ...
            <div class="relative">
              <input v-model="otp" type="text" inputmode="numeric" maxlength="6" placeholder="______"
                ...
                }" @input="validateOtpInput" @blur="otpTouched = true" />

              <!-- Ícone de feedback no OTP -->
              <div class="absolute inset-y-0 right-5 flex items-center">
                <svg v-if="otp.length === 6 && otp === requestOtp && !otpError" class="w-7 h-7 text-green-500"
                  fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                </svg>
                <svg v-if="otpError && otpTouched" class="w-7 h-7 text-red-500" fill="none" viewBox="0 0 24 24"
                  stroke="currentColor" stroke-width="2.5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                </svg>
              </div>
            ...
            <div v-if="otpError && otpTouched"
              class="mt-2 flex items-center justify-between bg-red-50 p-3 rounded-xl border border-red-100">
          ...
          <!-- Botão Voltar -->
          <button @click="resetOtpFlow"
            class="w-full py-4 bg-gray-100 text-gray-800 rounded-2xl font-bold text-lg hover:bg-gray-200 transition">
            Voltar
          </button>
        </div>
      </div>
      ...
      <!-- Links inferiores -->
      <div v-show="!showOtpInput" class="mt-10 text-center space-y-4">
        <router-link to="/" class="text-green-700 font-semibold text-lg hover:underline block">
...
const validateFullName = () => {
  ...
  if (!name) {
    fullNameError.value = 'O nome é obrigatório'
    fullNameValid.value = false
  } else if (name.length < 3) {
    fullNameError.value = 'O nome deve ter pelo menos 3 caracteres'
    fullNameValid.value = false
  } else if (/^\d/.test(name)) {
    fullNameError.value = 'O nome não pode começar com número'
    fullNameValid.value = false
  } else {
...
const handleSendOTP = async () => {
  ...
  try {
    ...
    error.value = 'Erro ao iniciar registro'
...
const verifyOTP = async () => {
  ...
  if (success) {
    ...
    error.value = store.state.error || 'Erro ao criar conta'
    setTimeout(() => error.value = '', 5000)
  }
}
...
// Limpar erro do telefone após tempo
watch(phoneError, (newError) => {
  if (newError && phoneTouched.value) {
    setTimeout(() => {
      phoneError.value = ''
    }, 4000)
  }
...
// Limpar erro do nome após tempo
watch(fullNameError, (newError) => {
  if (newError && fullNameTouched.value) {
    setTimeout(() => {
      fullNameError.value = ''
    }, 4000)
  }
...
const handleRegister = async () => {
  ...
  const query = await store.dispatch('checkPhoneExists', phone.value)

  if (query.exists) {
    loading.value = false
    error.value = 'Número de telefone já registado'
    setTimeout(() => error.value = '', 5000)
    return
  }
